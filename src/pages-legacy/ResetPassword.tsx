        ...
        >
          ...
          {clerkEnabled ? (
            <div className="rounded-[1.5rem] border border-white/10 bg-black/20 p-6 text-center">
              <p className="text-sm font-semibold text-foreground">Password reset is configured</p>
              <p className="mt-2 text-sm leading-6 text-muted-foreground">
                Use the reset link from Clerk&apos;s email flow to complete the password change, then return to sign in.
              </p>
            </div>
          ) : (
            <div className="rounded-[1.5rem] border border-dashed border-white/10 bg-black/20 p-6 text-center">
              <p className="text-sm leading-6 text-muted-foreground">
                Configure <code className="rounded bg-white/5 px-1.5 py-0.5 text-[11px]">NEXT_PUBLIC_CLERK_PUBLISHABLE_KEY</code> to enable Clerk password reset.
              </p>
            </div>
          )}
